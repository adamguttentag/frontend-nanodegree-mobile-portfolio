...
var pizzaElementGenerator = function(i) {
  ...
  //EDIT: Added pizzaImage class so we could select all pizzas and scale them
...
var resizePizzas = function(size) {
  ...
  // EDIT: replaced switches with two simple arrays. The arrays contain the values that would
  //       have been returned by the switch, and the slider value is used to indicate the
  //       index within the array. Less code and probably less logic for the browser to handle.
  //       This also allowed me to remove the changeSliderLabel function.
  // EDIT: created pizzaContainerWidth to indicate percentage width of the container.
  var pizzaContainerWidth = [0, 25, 33.3, 50];
  // EDIT: created pizzaSizeLabels to indicate label names
  var pizzaSizeLabels = ["","Small", "Medium", "Large"];
  // EDIT: substituted getElementById for querySelector to double performance. See:
  //       http://jsperf.com/getelementbyid-vs-queryselector
  document.getElementById("pizzaSize").innerHTML = pizzaSizeLabels[size];

  // EDIT: deleted determineDx and sizeSwitcher functions because they're not needed

  // Iterates through pizza elements on the page and changes their widths
  function changePizzaSizes(size) {
    // EDIT: replaced 3 instances of a DOM query with 1 that gets stored in a variable
    //       outside the for loop. Also changed querySelectorAll to getElementsByClassName.
    var randomPizzaContainers = document.getElementsByClassName("randomPizzaContainer");
    //var dx = determineDx(rPCs[0], size);
    //var newwidth = (rPCs[0].offsetWidth + dx) + 'px';

    for (var i = 0; i < randomPizzaContainers.length; i++) {
      // EDIT: deleted dx and width calculations because they're not needed and
      //       set the randomPizzaContainers to the pizzaContainerWidth percentage
      //       that was set by the slider switch.
      randomPizzaContainers[i].style.width = pizzaContainerWidth[size] + "%";
    }
  }
...
function updatePositions() {
  ...
  // EDIT: moved scrollTop out of the loop. No need to fetch for every pizza.
  var scrollTop = document.body.scrollTop / 1250;
  // EDIT: calculated phase values outside of the loop and stored them in an array
  var phaseValues = [];
  for (var i = 0; i < 5; i++) {
    // NOTE: limited the phaseValues to 3 decimal places for a small bump in performance.
    //       The extra precision is not needed and only results in the next for loop having to
    //       work with larger numbers, which requires more time and memory.
    phaseValues.push((Math.sin(scrollTop + i)).toFixed(3));
  }

  // EDIT: removed "var" from the for loop, as i already initialized in scope by the previous loop
  for (i = 0; i < window.items.length; i++) {
    //var phase = Math.sin(scrollTop + (i % 5));
    //EDIT: look up phase value from array calculated outside the loop
    var phase = phaseValues[i % 5];
    window.items[i].style.left = window.items[i].basicLeft + 100 * phase + 'px';

    //EDIT: considered using translate, then realized I'd have to position everything separately
    //items[i].style.transform = "translate(" + parseInt(items[i].basicLeft + 100 * phase) + "px)";
  }
...
document.addEventListener('DOMContentLoaded', function() {
  //EDIT: calculate max necessary number of pizzas.
  //      We need 1 column per 233 pixels of width, 1 row per 200 pixels of height.
  //      So we get the dimensions of the browser window, divide by the pixels per col/row
  //      and use toFixed(0) to avoid fractions of rows/columns.
  var rows = (window.innerHeight/200).toFixed(0);
  var cols = (window.innerWidth/233).toFixed(0);
  var s = 256;
  for (var i = 0; i < (rows * cols); i++) {
    ...
    //EDIT: rounded 73.333 pixels because there is no need to render .003 of a pixel.
